...
class RandomController:
        ...
        for i in self.driver.camera.getRecognitionObjects():
            ...
            if model_name == b'charger':
                ...
                if deviation <= 0.01 and deviation >= -0.01:
                    if depth <= -0.1:
                        self.driver.move('forward', speed=0.3)
                    else:
    ...
    def move_off_base_station(self):
        if self.state_data.time_counter <= 75:
            self.driver.move('backward', speed=randrange(1,3))
        elif self.state_data.time_counter <= 150:
            self.driver.turn('left', speed=randrange(1,3))
        else:
            self.transition_state(States.NAVIGATION)
            return
    ...
    def navigation_to_base_station(self):
        ...
        if (np.any(left_sensors_value < 1000)):
            self.driver.turn('right', speed=randrange(1,3), control=0)
        # if close to an object on the right, turn left
        elif (np.any(right_sensors_value < 1000)):
            self.driver.turn('left', speed=randrange(1,3), control=0)
        # else drive forward
        else:
            self.driver.move('forward', speed=randrange(1,3))
    ...
    def random_navigation(self):
        ...
        if self.state_data.reorienting:
            ...
            self.state_data.new_orientation = (current_orientation - random.randrange(15, 60)) % 360
    ...
    def turn_towards_pinecone(self):
        ...
        # check if pine cones are in detected objects
            ...
            for i in objects:
                ...
                # more than 4m
                if i.get_position()[2] < -4:
                    precision = 0
                # between 1m to 4m
                elif i.get_position()[2] >= -4 and i.get_position()[2] < -1:
                    precision = 0.5
                # less than 1m
