...
class Driver(Robot):
    ...
    def move(self, direction, speed, control=1.0):
        if direction == 'stop': inp = 0 
        if direction == 'forward': inp = speed 
        if direction == 'backward': inp = -speed
        self.wheels[0].setVelocity((randrange(100,120)/100) * inp * self.MAX_SPEED)
        self.wheels[1].setVelocity((randrange(100,120)/100) * inp * self.MAX_SPEED)
        self.wheels[2].setVelocity((randrange(100,150)/100) * inp * self.MAX_SPEED)
        self.wheels[3].setVelocity((randrange(100,150)/100) * inp * self.MAX_SPEED)
        self.wheels[4].setVelocity((randrange(100,110)/100) * inp * self.MAX_SPEED) 
        self.wheels[5].setVelocity((randrange(100,110)/100) * inp * self.MAX_SPEED)
    
    # define turning in specific direction
    def turn(self, direction, speed, control=1.0):
        if direction == 'stop': inp = 0 
        if direction == 'right': inp = speed 
        if direction == 'left': inp = -speed
        ...
        self.wheels[4].setVelocity(inp * self.MAX_SPEED * control) 
        self.wheels[5].setVelocity(-inp * self.MAX_SPEED * control)
    
    def state_of_robot(self):
        return self.step(self.TIME_STEP)
        
    def get_battery_value(self):
        return self.batterySensorGetValue()
